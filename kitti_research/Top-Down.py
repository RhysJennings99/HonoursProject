# -*- coding: utf-8 -*-
"""
Created on Wed May 12 12:04:51 2021
...
velo_points = load_from_bin(r'C:\Users\baboo\KITTI_Tutorial\velodyne_points\data\0000000000.bin')

print(velo_points.shape)

def normalize_depth(val, min_v, max_v):
    """ 
    This function is used to normalize the depth value so we can visualise the colour of close points and far points. 
...
def in_range_points(points, x, y, z, x_range, y_range, z_range):
    """ Extracts all the points in the range that we are looking for"""
...
def velo_points_2_top_view(points, x_range, y_range, z_range, scale):
    ...
    # Extracting the points and calculating the 2D distance.
    x = points[:, 0]
    y = points[:, 1]
    z = points[:, 2]
